...
### 검증 체크포인트
- [x] 04-09 10:00 정밀폐기 세션에서 46704 포함 4매장 보고서 파일 정시 생성 (세션 시작 ±2분)
  - (완료: 04-09, 46704 10:11:05 생성, 슬립 4건 수집, `[VerifyDeep] 46704 2026-04-09: L1=OK`) ✅
- [x] 04-09 14:00 정밀폐기 세션 동일 검증
  - (완료: 04-09, 4/4 매장 14:11 생성, `[ExpiryConfirm+WasteSlip(14:00)] 완료: 4/4 성공`) ✅
- [ ] 04-09 22:00 정밀폐기 세션 동일 검증
  - ⚠️ 미검증: 14:42 스케줄러 auto-reload 후 04-10 00:05까지 미복구 (약 9시간 다운). 22:00 세션 로그 없음.
  - → scheduling.md#스케줄러-장기-다운 신규 이슈 등록 필요
- [ ] 04-09 23:55 ops_metrics 실행에서 `verification_log_files_missing = 0`
  - ⚠️ 미검증: 동일 스케줄러 다운으로 23:55 ops_issue_pipeline 미실행
- [x] 만약 여전히 silent exception이 발생한다면, 새 exc_info stack trace로 **진짜 원인**(예: DB lock, 넥사크로 프레임 전환 실패) 식별 가능해야 함
  - (완료: 04-09 전체 silent exception 0건, `[WasteSlip] 수집 오류` / `[VerifyDeep] 보고서 생성 실패` 없음) ✅
- [ ] 2주 연속(04-09 ~ 04-22) 동일 매장 재발 0건 → [RESOLVED]
  - (진행 중 — 1일차 04-09 확인됨, 계속 모니터링)
- [x] logs에서 `[VerifyDeep] {store_id}` 형식으로 매장 식별 가능 확인
  - (완료: `[VerifyDeep] 46704 2026-04-09: L1=OK` 형식 정상 출력) ✅

### 04-09 검증 요약 (자동 검증, 04-10)
- **핵심 결과**: 가드 완화 수정(b5d458a) 정상 작동 확인. 46704 10:11 보고서 정시 생성, 슬립 4건 정상 수집.
- **검증 통과**: 10:00 세션, 14:00 세션, silent exception 0건, store_id 로그 형식
- **검증 불가**: 22:00 세션, 23:55 ops_metrics — 스케줄러 auto-reload(14:42) 후 9시간 다운으로 인한 미실행 (fix 자체 실패 아님)
- **신규 이슈**: 스케줄러 9시간 다운 → scheduling.md 등록 필요
...
## [OPEN] BatchSync 가드 우회 건수 18→250건 급증 (04-10) — 재진입 패턴 의심 (P1)

**목표**: 04-10 야간 배치(21:51~21:52)에서 false_consumed_post_guard 4회 연속 탐지(21건+73건+64건+92건=250건). 동일 상품코드가 여러 이벤트에 중복 등장 → 단일 배치가 반복 처리되는 재진입(re-entry) 패턴 의심.
**동기**: 자동 감지 (2026-04-10) — false_consumed_post_guard 지표. 기존 [PLANNED] 18건(P2, 04-08) → 14배 폭증으로 P1 긴급 상향
**근거**:
- 04-08 탐지: 18건 → 04-10: 21+73+64+92=250건 (동일 날 4건 발생)
- 중복 상품코드: 8800271902513, 8801771304173, 8800279675761 등이 여러 이벤트에 반복 등장
- 타임스탬프 집중: 21:51:48 ~ 21:52:04 (16초 내 4이벤트) → 야간 정리 로직 일괄 트리거 의심
- AI 분석: "단일 배치 반복 처리 재진입 경로" 가설 (2026-04-11_response.md)
**영향**: 잘못된 consumed 마킹 → remaining_qty 부풀어 폐기율 과보고 / 실제 과발주 이중 작용 (001~003 폐기율 폭등과 연결)
**예상 수정 방향**: BatchSync(재고 정합성 동기화) 서비스의 야간 정리 로직에서 재진입 방지 여부 확인. `sale_qty > 0`인 경우 FIFO 차감 허용하는 가드 조건 완화 검토.

### 검증 체크포인트
- [ ] 04-11 07:00 스케줄 실행 후 동일 패턴(21:51 집중) 재현 여부 확인
- [ ] DB에서 중복 상품코드 배치의 `updated_at` 패턴 조회 — 재진입 경로 확인
- [ ] 수정 후 false_consumed_post_guard = 0건 → [RESOLVED]
