...
### 시도 2: 일요일 비식품 기본값 발주 차단 (1da6016, 04-12)
- **왜**: 시도1만으로 부족 — 46513도 Direct API 경로에서 orderable_day 미수집 상품 7건 발주됨
  - 실제 BGF 수집 데이터 기준: 식품(001~005)/샐러드(007) 외 **100% 일요일 불가**
  - 기본값("일월화수목금토")은 미수집을 의미하므로 비식품은 일요일 차단이 안전
- **수정**: `is_orderable_today()`: 일요일 + 기본값 + 비식품(mid∉001~005,007) → False
- **결과**: ✓ 7건 모두 차단 확인 (시뮬레이션)

### 검증 체크포인트
- [x] common.db v77 마이그레이션 적용 확인 (04-12)
- [x] 매장별 분리 저장 기능 테스트 통과 (04-12)
- [x] 일요일 비식품 기본값 차단 로직 검증 (04-12, 시뮬레이션)
- [ ] 46704 product_details 자동 수집 확인 (04-13 07:00 스케줄)
- [ ] 46704+46513 일요일 발주불가 상품 필터링 확인 (다음 일요일 04-19)
...
- 기본값 = 미수집. 기본값을 "모든 요일 가능"으로 해석하면 비식품에서 오발주 발생
...
**동기**: 자동 감지 (2026-04-12) -- waste_rate_non_food_surge
**선행조건**: 없음
**예상 영향**: waste_rate_non_food_surge 관련 파일
