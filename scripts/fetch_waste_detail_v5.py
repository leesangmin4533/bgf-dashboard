"""
폐기 전표 상세 품목 추출 (v5) - dsGsTmp 직접 수정 + fn_selSearch 재호출

핵심: 팝업의 dsGsTmp가 첫 번째 전표값으로 고정되어 서버 트랜잭션이 같은 데이터만 반환
해결: 팝업 오픈 후 dsGsTmp의 gvVar07(전표번호), gvVar06(날짜) 등을 갱신하고 fn_selSearch() 재호출
"""
...
def main():
    ...
    try:
        ...
        # ============================================================
        # 각 전표에 대해: dsGs 설정 -> 팝업 열기 -> dsGsTmp 갱신 -> fn_selSearch -> 데이터 추출
        # ============================================================
        all_items = []
        ...
        for idx in range(len(slip_list)):
            ...
            chit_no = str(slip.get("CHIT_NO", ""))
            chit_id = str(slip.get("CHIT_ID", ""))
            chit_id_nm = str(slip.get("CHIT_ID_NM", ""))
            chit_id_no = str(slip.get("CHIT_ID_NO", ""))
            item_cnt = slip.get("ITEM_CNT", 0) or 0
            nap_ymd = str(slip.get("NAP_PLAN_YMD", ""))
            center_nm = str(slip.get("CENTER_NM", ""))
            chit_flag = str(slip.get("CHIT_FLAG", ""))

            print(f"  [{idx}] {date_str} 전표={chit_no} 품목={item_cnt}건", end=" ")
            ...
            driver.execute_script(f"""
                try {{
                    ...
                    pf.dsGsTmp.setColumn(0, 'gvVar07', '{chit_no}');
            ...
            if items and not items.get("error"):
                ...
                gs_var07 = items.get("dsGsTmp_gvVar07", "")

                if item_list:
                    print(f"-> {len(item_list)}건 (gvVar07={gs_var07})")
                    for r in item_list:
                    ...
                    print(f"-> 품목 0건 (gvVar07={gs_var07})")
            else:
                print(f"-> 오류: {items}")
        ...
        # ============================================================
        print(f"\n\n{'='*80}")
        print(f"  2/{FROM_DATE[6:8]}~2/{TO_DATE[6:8]} 폐기 전표 상세 품목")
        print(f"{'='*80}")
        ...
        if all_items:
            ...
            for date_str in sorted(by_date.keys()):
                items_for_date = by_date[date_str]
                total_wonga = sum(i.get("wonga_amt", 0) or 0 for i in items_for_date)
                total_maega = sum(i.get("maega_amt", 0) or 0 for i in items_for_date)
                print(f"\n  === {date_str} === {len(items_for_date)}건 | 원가 {total_wonga:,.0f}원 | 매가 {total_maega:,.0f}원")
