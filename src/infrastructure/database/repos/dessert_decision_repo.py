"""
DessertDecisionRepository -- 디저트 발주 유지/정지 판단 결과 저장소 (store DB)

dessert_decisions 테이블 CRUD를 담당합니다.
"""
...
class DessertDecisionRepository(BaseRepository):
    """디저트 발주 유지/정지 판단 결과 저장소"""
    ...
    def save_decisions_batch(self, results: List[Dict[str, Any]]) -> int:
        ...
        for r in results:
            try:
                ...
                logger.warning(f"dessert_decisions 저장 실패 ({r.get('item_cd')}): {e}")
    ...
    def get_latest_decisions(
        ...
        dessert_category: Optional[str] = None,
    ) -> List[Dict[str, Any]]:
        """각 상품별 최신 판단 결과 조회"""
        ...
        if dessert_category:
            ...
            params.append(dessert_category)
    ...
    def get_confirmed_stop_items(self, store_id: Optional[str] = None) -> Set[str]:
        """운영자가 CONFIRMED_STOP으로 확인한 상품 코드 반환 (OrderFilter용)"""
        sid = store_id or self.store_id
        conn = self._get_conn()
    ...
    def get_stop_recommended_items(self, store_id: Optional[str] = None) -> Set[str]:
        """STOP_RECOMMEND 상태인 상품 코드 반환 (대시보드 표시용)"""
        sid = store_id or self.store_id
        conn = self._get_conn()
    ...
    def get_reduce_order_items(self, store_id: Optional[str] = None) -> Set[str]:
        """REDUCE_ORDER 상태인 상품 코드 반환 (improved_predictor 감량용)"""
        sid = store_id or self.store_id
        conn = self._get_conn()
    ...
    def get_decisions_by_period(
        ...
        query = "SELECT * FROM dessert_decisions WHERE store_id = ? AND category_type = 'dessert'"
        params: list = [sid]

        if start_date:
            query += " AND judgment_period_end >= ?"
            params.append(start_date)
        if end_date:
            query += " AND judgment_period_end <= ?"
            params.append(end_date)

        query += " ORDER BY judgment_period_end DESC, item_cd"
        cursor = conn.execute(query, params)
    ...
    def update_operator_action(
        ...
        Args:
            decision_id: dessert_decisions.id
        ...
        try:
            ...
            logger.error(f"operator_action 업데이트 실패 (id={decision_id}): {e}")
            return False
    ...
    def batch_update_operator_action(
        ...
        Args:
            item_cds: 상품코드 리스트 (최대 50개)
            action: 'CONFIRMED_STOP' 또는 'OVERRIDE_KEEP'
            operator_note: 운영자 메모
            store_id: 매장 ID (기본값: self.store_id)
        ...
        # 최신 판단 ID 조회 (STOP_RECOMMEND + 미처리)
        ...
        cursor = conn.execute(f"""
            ...
            WHERE d.store_id = ?
              ...
              AND d.decision IN ('STOP_RECOMMEND', 'REDUCE_ORDER')
              AND (d.operator_action IS NULL OR d.operator_action NOT IN ('OVERRIDE_KEEP'))
    ...
    def get_weekly_trend(
        ...
        Returns:
            [{"week": "W4", "KEEP": 102, "WATCH": 18, "STOP_RECOMMEND": 2}, ...]
        """
        sid = store_id or self.store_id
        conn = self._get_conn()
        ...
        cursor = conn.execute("""
            ...
            FROM dessert_decisions
            WHERE store_id = ?
              AND category_type = 'dessert'
              AND judgment_period_end >= date('now', ? || ' days')
              AND decision IN ('KEEP', 'WATCH', 'REDUCE_ORDER', 'STOP_RECOMMEND')
        ...
        for row in cursor.fetchall():
            ...
            if wn not in week_data:
                week_data[wn] = {"KEEP": 0, "WATCH": 0, "REDUCE_ORDER": 0, "STOP_RECOMMEND": 0}
    ...
    def get_decision_summary(
        ...
        """카테고리별 판단 결과 집계 (대시보드 위젯용)

        Returns:
            {
                "current": {"KEEP": n, "WATCH": n, "STOP_RECOMMEND": n, "SKIP": n},
                "by_category": {"A": {"KEEP": n, ...}, ...},
            }
        """
        sid = store_id or self.store_id
        conn = self._get_conn()
        ...
        current: Dict[str, int] = {
            "KEEP": 0, "WATCH": 0, "REDUCE_ORDER": 0, "STOP_RECOMMEND": 0, "SKIP": 0,
        }

        for row in cursor.fetchall():
            cat, decision, cnt = row
            if cat not in by_category:
                by_category[cat] = {"KEEP": 0, "WATCH": 0, "REDUCE_ORDER": 0, "STOP_RECOMMEND": 0}
            by_category[cat][decision] = cnt
            # current 집계
            if decision in current:
                current[decision] += cnt

        # SKIP 계산: 전체 디저트 상품 수 - 판단된 상품 수
        decided_count = (current["KEEP"] + current["WATCH"]
                         + current["REDUCE_ORDER"] + current["STOP_RECOMMEND"])
        try:
            total_cursor = conn.execute("""
                SELECT COUNT(DISTINCT item_cd) FROM dessert_decisions
                WHERE store_id = ? AND category_type = 'dessert'
            """, (sid,))
            total_items = total_cursor.fetchone()[0]
            current["SKIP"] = max(0, total_items - decided_count)
        except Exception:
            current["SKIP"] = 0
