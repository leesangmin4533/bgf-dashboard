"""
AutoOrderItemRepository — 자동발주 상품 캐시 저장소

원본: src/db/repository.py AutoOrderItemRepository (lines 4247-4357)
"""
...
class AutoOrderItemRepository(BaseRepository):
    ...
    def get_all_item_codes(self, store_id: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            rows = conn.execute(
                f"SELECT item_cd FROM auto_order_items WHERE 1=1 {sf}",
                sp
            ).fetchall()
    ...
    def get_all_detail(self, store_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            rows = conn.execute(
                ...
                f"FROM auto_order_items WHERE 1=1 {sf} ORDER BY item_cd",
                sp
            ).fetchall()
            return [
                {
    ...
    def refresh(self, items: List[Dict[str, str]], store_id: Optional[str] = None) -> int:
        ...
        try:
            ...
            # 해당 매장 데이터만 삭제
            sf, sp = self._store_filter(None, store_id)
            cursor.execute(f"DELETE FROM auto_order_items WHERE 1=1 {sf}", sp)

            if not items:
                conn.commit()
                logger.info("자동발주 상품 0건 - 기존 캐시 삭제 완료")
                return 0
    ...
    def get_count(self, store_id: Optional[str] = None) -> int:
        ...
        try:
            sf, sp = self._store_filter(None, store_id)
            row = conn.execute(
                f"SELECT COUNT(*) FROM auto_order_items WHERE 1=1 {sf}",
                sp
            ).fetchone()
    ...
    def get_last_updated(self, store_id: Optional[str] = None) -> Optional[str]:
        ...
        try:
            sf, sp = self._store_filter(None, store_id)
            row = conn.execute(
                f"SELECT MAX(updated_at) FROM auto_order_items WHERE 1=1 {sf}",
                sp
            ).fetchone()
