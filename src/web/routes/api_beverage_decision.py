"""음료 발주 유지/정지 판단 REST API.

GET  /api/beverage-decision/latest              -- 최신 판단 결과
GET  /api/beverage-decision/history/<item_cd>    -- 상품별 이력
GET  /api/beverage-decision/summary             -- 카테고리별 집계 (+history 파라미터)
POST /api/beverage-decision/action/<decision_id> -- 운영자 액션
...
@beverage_decision_bp.route("/latest", methods=["GET"])
def get_latest_decisions():
    ...
    try:
        ...
        repo = BeverageDecisionRepository(store_id=store_id)
        ...
        logger.error(f"[BeverageDecisionAPI] latest 조회 실패: {e}")
        return jsonify({"success": False, "error": str(e)}), 500


@beverage_decision_bp.route("/history/<item_cd>", methods=["GET"])
def get_item_history(item_cd: str):
    ...
    try:
        ...
        logger.error(f"[BeverageDecisionAPI] history 조회 실패: {e}")
        return jsonify({"success": False, "error": str(e)}), 500


@beverage_decision_bp.route("/summary", methods=["GET"])
def get_summary():
    ...
    try:
        ...
        logger.error(f"[BeverageDecisionAPI] summary 조회 실패: {e}")
        return jsonify({"success": False, "error": str(e)}), 500


@beverage_decision_bp.route("/action/<int:decision_id>", methods=["POST"])
def update_action(decision_id: int):
    ...
    try:
        ...
        logger.error(f"[BeverageDecisionAPI] action 업데이트 실패: {e}")
        return jsonify({"success": False, "error": str(e)}), 500


@beverage_decision_bp.route("/action/batch", methods=["POST"])
def batch_action():
    """일괄 운영자 확인."""
    data = request.get_json(silent=True) or {}
    ...
    try:
        ...
        logger.error(f"[BeverageDecisionAPI] batch action 실패: {e}")
        return jsonify({"success": False, "error": str(e)}), 500


@beverage_decision_bp.route("/run", methods=["POST"])
def run_decision():
    """수동 실행"""
    store_id = _get_store_id()
    data = request.get_json(silent=True) or {}
    ...
    try:
        ...
        flow = BeverageDecisionFlow(store_id=store_id)
        result = flow.run(
        ...
        logger.error(f"[BeverageDecisionAPI] 수동 실행 실패: {e}")
        return jsonify({"success": False, "error": str(e)}), 500
