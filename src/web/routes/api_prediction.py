...
def prediction_summary():
    ...
    data = {
        ...
        "alerts": _get_alerts(store_id=store_id),
    }
...
def _get_qty_accuracy(store_id=None):
    ...
    try:
        ...
        return {
            ...
            "smape": round(metrics.smape, 1),
            "wmape": round(metrics.wmape, 1),
            "accuracy_within_1": round(metrics.accuracy_within_1, 1),
            "accuracy_within_2": round(metrics.accuracy_within_2, 1),
            "accuracy_within_3": round(metrics.accuracy_within_3, 1),
            "over_rate": round(metrics.over_prediction_rate, 1),
            "under_rate": round(metrics.under_prediction_rate, 1),
            "avg_over_amount": round(metrics.avg_over_amount, 2),
            "avg_under_amount": round(metrics.avg_under_amount, 2),
        }
    except Exception as e:
        ...
        return {
            ...
            "smape": 0, "wmape": 0,
            "accuracy_within_1": 0, "accuracy_within_2": 0, "accuracy_within_3": 0,
...
def _get_group_quantile_alpha():
    """카테고리 그룹별 quantile alpha 설정 (trainer.py 소스에서 import)"""
    try:
        from src.prediction.ml.trainer import GROUP_QUANTILE_ALPHA
        return dict(GROUP_QUANTILE_ALPHA)
...
def _get_alerts(store_id=None):
    """예측 알림 조건 체크 (AccuracyReporter.check_alerts 활용)"""
    try:
        from src.prediction.accuracy.reporter import AccuracyReporter
        reporter = AccuracyReporter(store_id=store_id)
        metrics = reporter.tracker.get_accuracy_by_period(7)
        categories = reporter.tracker.get_accuracy_by_category(7)
        return reporter.check_alerts(metrics, categories)
    except Exception as e:
        logger.warning(f"알림 조회 실패: {e}")
        return []
