"""
DessertDecisionFlow -- 디저트 판단 Use Case (스케줄러에서 호출)

Usage:
    flow = DessertDecisionFlow(store_id="46513")
    ...
    def run(
        ...
        **kwargs,
        ...
        if not self.store_id:
            logger.error("[DessertDecisionFlow] store_id 미지정")
            return {"error": "store_id required"}

        service = DessertDecisionService(store_id=self.store_id)
        return service.run(
