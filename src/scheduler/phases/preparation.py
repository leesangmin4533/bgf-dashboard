...
(발주 정합성 검증은 10:30 pending_sync에서 수행)
"""
...
def run_preparation_phases(ctx: Dict[str, Any], job: Any) -> Dict[str, Any]:
    ...
    if run_auto_order and collection_success:
        try:
            with phase_timer("1.68", "DirectAPI Realtime Stock Refresh",
    ...
    if collection_success and not run_auto_order:
        try:
            with phase_timer("1.7", "Prediction Logging",
                             store_id=store_id, _logger=logger,
                             timings=ctx.get("_phase_timings")):
    ...
    if run_auto_order and collection_success and not ot_sync_done:
        try:
            with phase_timer("1.95", "Order Status -> OT Sync",
                             store_id=store_id, _logger=logger,
                             timings=ctx.get("_phase_timings")):
                driver = job.collector.get_driver()
                if driver:
    ...
    # Phase 1.96 제거: 발주 정합성 검증은 10:30 pending_sync에서 수행
    # 7시에는 발주현황 화면이 닫혀있어 항상 실패하고,
    # 조회 실패 시 어제 발주 전체를 무효화하는 이중발주 버그 유발 (2026-04-04 49965 89건 사례)
