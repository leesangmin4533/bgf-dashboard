...
CREATE TABLE IF NOT EXISTS milestone_snapshots (
    ...
    """,
    74: """
-- v74: job_runs 스케줄 잡 실행 로그 (common.db, job-health-monitor feature)
...
ALTER TABLE prediction_logs ADD COLUMN stage_trace TEXT;
    ...
    77: """
-- v77: product_details PK를 (store_id, item_cd)로 변경 — 매장별 orderable_day 분리
-- 46704 매장에서 일요일 발주 불가 상품이 46513 데이터 참조로 발주되는 문제 해결.
-- SQLite는 ALTER TABLE로 PK 변경 불가 → 테이블 재생성 방식.
